...
void print_parameters(Params& para) {
  ...
  cout.width(28);
  cout << left << "sigma " << right << setw(3) << para.sigma << endl;
  cout.width(28);
  cout << left << "Max_lip_i " << right << para.lip << endl;
  cout.width(28);
  cout << left << "h " << right << para.h << endl;
  cout.width(28);
  cout << left << "n " << right << para.n << endl;
  cout.width(28);
  cout << left << "m " << right << para.m << endl;
  cout.width(28);
  cout << "---------------------------------" << endl;  
}
...
void parse_input_argv(Params* para, int argc, char *argv[]){
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (std::string(argv[i - 1]) == "-tol"){
			para->tol = atof(argv[i]);
		}
